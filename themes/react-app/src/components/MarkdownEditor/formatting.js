import { grey400 } from "material-ui/styles"
...
const isEmptyOneLineSelection = (line, length) => isEmpty(line) && length === 1
...
const formatMultiline = signature => cm => () => {
  ...
  Array(length)
    ...
    .forEach((from, i) => {
      ...
      let text =
        signature +
        (isEmptyOneLineSelection(line, length)
          ? getPlaceholderBySignature(signature)
          : line)
      if (signature === "#. ") {
  ...
  codeMirror.setSelection(
    ...
    position(end.line, lastLineLength)
  )
  codeMirror.focus()
}
...
const removeInline = signature => cm => () => {
  ...
  const [start, end] = getPositions(cursor.ch, getMatches(signature, line))
  ...
  codeMirror.replaceRange(text, startPoint, endPoint)
}
...
export const getStyleIfActive = tokens => token =>
  isActiveToken(token, tokens) ? { backgroundColor: grey400 } : {}
